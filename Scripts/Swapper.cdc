...
pub contract SwapperContract {
    ...
    pub event Swapped (flowAmount: UFix64, DNMAmount: UFix64 , by: Address)
    ...
    pub resource Swapper{
        ...
        pub fun Swap (from: &FungibleToken.Vault, amount: UFix64): @FungibleToken.Vault {
            ...
            let minter = SwapperContract.account.borrow<&Dynamo.Admin>(from: Dynamo.AdminStoragePath) ?? panic("Swapper Contract must be deployed to DYNAMO Admin address")
